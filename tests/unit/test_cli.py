...
def test_standard_parser_uses_documented_long_options_only():
    args = build_parser().parse_args(
        ["standard", "--input", "input.csv", "--output-dir", "output", "--db-path", "db",
         "--tmp-dir", "tmp", "--work-dir", "work"]
    )
    ...
    assert not hasattr(args, "mmseqs")
    assert not hasattr(args, "cluster_identity")


@pytest.mark.parametrize("arguments", [["standard", "input.csv", "output"], ["standard", "--db", "db"]])
...
def test_accelerated_parser_owns_accelerated_options():
    args = build_parser().parse_args(
        ["accelerated", "--input", "input.csv", "--output-dir", "output", "--db-path", "db",
         "--mmseqs", "mmseqs", "--cluster-identity", "0.9", "--cluster-coverage", "0.8",
         "--cluster-mode", "1"]
    )
